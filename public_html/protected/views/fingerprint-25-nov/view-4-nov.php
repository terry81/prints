...
<?php
	foreach($model->truepositives as $relatedModel) {
        ...
        echo '<tr><td><a href="http://www.uniprot.org/uniprot/'.$protein->code.'">'.$protein->code.'</a></td>'.'<td>'.$truepositive->accession_number.'</td></tr>';
		//echo GxHtml::closeTag('li');
	}
?>
</table>
<?php
    ...
    if (!empty($tpp_array)) {
        ...
        //The link to Uniprot is connected to the accession number as requested by prof. Attwood
            echo '<tr><td><a href="http://www.uniprot.org/uniprot/'.$val[1].'">'.$key.'</a></td><td>'.$val[1].'</td><td>'.$val[0].'</td>';
        }
        echo '</table></br></br>';
    }
?>
